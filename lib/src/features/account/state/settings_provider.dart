...
class SettingsNotifier extends ChangeNotifier {
    ...
    try {
      switch (settingType) {
        ...
        case 'percentOff':
          ...
          break;
        case 'hottestDeals':
          _settingsData?.setHottestDealsNotification = setting;
          if (setting == true) {
            _firebaseMessaging.subscribeToTopic('hottest_deals');
          } else {
            _firebaseMessaging.unsubscribeFromTopic('hottest_deals');
          }
          break;
        case 'whatsappDeals':
          _settingsData?.setWhatsappNotification = setting;
          if (setting == true) {
            _firebaseMessaging.subscribeToTopic('whatsapp_deals');
          } else {
            _firebaseMessaging.unsubscribeFromTopic('whatsapp_deals');
          }
          break;
      }
      notifyListeners();
  ...
  void updateTopicSubscriptions() {
    ...
    if (_settingsData?.hottesdealsNotification == true) {
      _firebaseMessaging.subscribeToTopic('hottest_deals');
    }

    if (_settingsData?.whatsappNotification == true) {
      _firebaseMessaging.subscribeToTopic('whatsapp_deals');
    }
  }
  ...
  bool isIdenticalSettings(
    ...
    return cachedSettings.priceMistake == serverSettings.priceMistake &&
        ...
        cachedSettings.hottesdealsNotification ==
            serverSettings.hottesdealsNotification &&
        cachedSettings.whatsappNotification ==
            serverSettings.whatsappNotification;
  }
}
